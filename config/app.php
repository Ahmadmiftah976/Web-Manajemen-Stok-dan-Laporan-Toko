...
// ── 1. Load .env ──────────────────────────────────────────────────────────────
// Jika file .env ada (lokal/XAMPP), baca dari file.
// Jika tidak ada (Vercel/production), baca dari environment sistem yang sudah
// diset melalui dashboard Vercel (Settings → Environment Variables).
...
if (file_exists($envFile)) {
    ...
    foreach ($lines as $line) {
        ...
        if (str_contains($line, '=')) {
            ...
            // Set ke $_ENV dan getenv() (hanya jika belum diset oleh sistem)
            if (getenv($key) === false) {
                putenv("$key=$value");
                $_ENV[$key] = $value;
            }
...
// Jika .env tidak ditemukan, environment variables dibaca langsung dari sistem
// (sudah diinjeksi oleh Vercel secara otomatis)
