...
class KasirController extends Controller
{
    ...
    public function index(): void
    {
        ...
        // Ringkasan hari ini
        $todaySummary = $this->transactionModel->getTodaySummary((int) $warehouseId, (int) Auth::user('id'));
    ...
    public function checkout(): void
    {
        ...
        if (!$payload || empty($payload['cart'])) {
            $this->jsonError('Keranjang kosong atau format data tidak valid');
        }
        ...
        // Validasi Gudang
        if ($warehouseId <= 0) {
            $this->jsonError('Gudang harus dipilih.');
        }
        ...
        if ($transactionId) {
            $this->jsonSuccess('Transaksi berhasil disimpan!', ['transaction_id' => $transactionId]);
        } else {
            $this->jsonError('Gagal memproses transaksi. Pastikan stok mencukupi.', 500);
        }
    }
